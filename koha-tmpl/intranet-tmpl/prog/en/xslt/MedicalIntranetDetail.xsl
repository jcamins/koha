   ...
   <xsl:if test="$materialTypeCode!=''">
       ...
       <xsl:if test="marc:datafield[@tag=020]">
        ...
        <xsl:if test="marc:datafield[substring(@tag, 1, 1) = '6' and @ind2 = '2']">
            <span class="results_summary subjects"><span class="label">Medical subject(s): </span>
            ...
            <xsl:otherwise>
               <xsl:attribute name="href">/cgi-bin/koha/catalogue/search.pl?q=mesh<xsl:value-of select="$SubjectModifier"/>:<xsl:value-of select="$TracingQuotesLeft"/><xsl:value-of select="marc:subfield[@code='a']"/><xsl:value-of select="$TracingQuotesRight"/></xsl:attribute>
            </xsl:otherwise>
            </xsl:choose>
            <xsl:call-template name="chopPunctuation">
                <xsl:with-param name="chopString">
        ...
        <xsl:if test="marc:datafield[substring(@tag, 1, 1) = '6' and @ind2 != '2']">
            ...
            <xsl:otherwise>
               <xsl:attribute name="href">/cgi-bin/koha/catalogue/search.pl?q=lcsh<xsl:value-of select="$SubjectModifier"/>:<xsl:value-of select="$TracingQuotesLeft"/><xsl:value-of select="marc:subfield[@code='a']"/><xsl:value-of select="$TracingQuotesRight"/></xsl:attribute>
            </xsl:otherwise>
            </xsl:choose>
            <xsl:call-template name="chopPunctuation">
                <xsl:with-param name="chopString">
