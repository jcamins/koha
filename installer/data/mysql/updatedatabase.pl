...
$DBversion = "3.06.08.000";
if (C4::Context->preference("Version") < TransformToNum($DBversion)) {
    print "Upgrade to $DBversion done (Incrementing version for 3.6.8 release. See release notes for details.) \n";
    SetVersion ($DBversion);
}

$DBversion = "3.06.09.000";
if (C4::Context->preference("Version") < TransformToNum($DBversion)) {
    print "Upgrade to $DBversion done (Incrementing version for 3.6.9 release. See release notes for details.) \n";
    SetVersion ($DBversion);
}

$CPversion = "3.06.10.000";
if (C4::Context->preference("CPVersion") < TransformToNum($CPversion)) {
    print "Upgrade to $CPversion done (Incrementing version for 3.6.10-cpspecial release. See release notes for details.) \n";
    SetVersion ($CPversion);
}
